...
//symbol_entry symbol_table[1000];
assembly_code list_element;
//symbol_entry symbol_index;
...
//void print_symbol_table(symbol_entry table[]);
...
void Declaration_list_v2(){
  ...
  if(token1.lexeme_word == ";"){
    ...
    if(token1.lexeme_word == "integer" || token1.lexeme_word == "boolean"){
      //symbol_index.iden = token1.lexeme_word;
      Declaration_list_v2();
    }
    else {
      backup = token1;
...
void Qualifier_v2(){
  if(token1.lexeme_word == "integer" || token1.lexeme_word == "boolean"){
    //symbol_index.data_type = token1.lexeme_word;
    token1 = Lexer();
  }
  else {
...
void IDs_v2(){
  ...
  //   symbol_index.iden = token1.lexeme_word;
  //   symbol_table[sym_index_num] = symbol_index;
  //   sym_index_num++;
    token1 = Lexer();
    if(token1.lexeme_word == ","){
      token1 = Lexer();
...
//   int j = 0;
//   outputfile << "\n----SYMBOL TABLE----\n";
//   while(table[j].iden != "0"){
//     temp2 = table[j];
//     outputfile << temp2.iden << " " << temp2.sym_address << " " << temp2.data_type << "\n";
//     j++;
//   }
// }
